...
function MarketBasketAnalysis() {
  console.log("MarketBasketAnalysis component rendering...");
  ...
  const [minSupport, setMinSupport] = useState(0.01); // Lowered from 0.05
  const [minConfidence, setMinConfidence] = useState(0.2); // Lowered from 0.5
  const [error, setError] = useState(null);

  console.log("MarketBasketAnalysis state:", {
    loading,
    error,
    rulesCount: associationRules.length,
    opportunitiesCount: crossSellOpportunities.length,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        // Perform market basket analysis using backend API
        ...
        console.log("Market basket analysis result:", analysisResult);
        ...
        // Process association rules for frontend display
        let processedRules = [];
        if (
          analysisResult.data.association_rules &&
          analysisResult.data.association_rules.length > 0
        ) {
          processedRules = analysisResult.data.association_rules.map(
            (rule, index) => {
              // Dynamic revenue calculation based on product types and rule strength
              const getProductValue = (items) => {
                ...
                itemArray.forEach((item) => {
                  ...
                  if (
                    itemLower.includes("iphone") ||
                    itemLower.includes("macbook")
                  ) {
                    baseValue += 800;
                  } else if (
                    itemLower.includes("gaming") ||
                    itemLower.includes("playstation")
                  ) {
                    baseValue += 400;
                  } else if (
                    ...
                    baseValue += 200;
                  } else if (itemLower.includes("smart")) {
                    baseValue += 150;
                  } else {
                    baseValue += 50;
                  }
              ...
              const antecedentValue = getProductValue(rule.antecedent);
              const consequentValue = getProductValue(rule.consequent);
              const avgProductValue = (antecedentValue + consequentValue) / 2;

              // Calculate dynamic revenue based on support, confidence, lift, and product value
              const baseRevenue = avgProductValue * rule.support * 1000; // Scale factor
              const confidenceMultiplier = 0.5 + rule.confidence * 0.5; // 0.5 to 1.0
              const liftMultiplier = Math.min(rule.lift / 10, 2); // Cap at 2x

              const dynamicRevenue = Math.round(
                baseRevenue * confidenceMultiplier * liftMultiplier
              );
              ...
              return {
                ...
                transactions: Math.round(rule.support * 1000), // More realistic transaction count
                expectedRevenue: dynamicRevenue, // Dynamic revenue calculation
                strength:
                  rule.lift > 3
                    ? "Very Strong"
                    : rule.lift > 2
          ...
          // Generate mock association rules when none are found
          console.log(
            "No association rules found, generating mock data for demonstration"
          );
          processedRules = [
            {
        ...
        // Generate cross-sell opportunities from association rules
        let crossSellOps = [];
        if (processedRules.length > 0) {
          ...
          // Fallback cross-sell opportunities
          crossSellOps = [
            {
            ...
            },
            {
              ...
              recommendations: ["Coffee Beans", "Coffee Filters"],
        ...
        // Generate customer segments from product categories
        const customerSegs = [
          {
          ...
          },
          {
            ...
            size: 15,
            avgOrderValue: 67.3,
            topProducts: ["Basic Items", "Utility Products"],
            behaviorPattern: "Value-conscious purchasing behavior",
          },
        ];
        ...
        console.log("Association rules:", processedRules.length);
        console.log("Cross-sell opportunities:", crossSellOps.length);
      } catch (err) {
        console.error("Error loading market basket data:", err);
        console.error("Error stack:", err.stack);
        setError(`Failed to load market basket analysis: ${err.message}`);
      } finally {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Analyzing purchase patterns and generating association rules...
              </p>
            </div>
          </div>
        </div>
  ...
  if (!associationRules || !crossSellOpportunities || !customerSegments) {
    return (
      <div className="p-8">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
          ...
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              <RefreshCw className="h-8 w-8 animate-spin text-blue-600 mx-auto mb-4" />
              <p className="text-gray-600">Processing market basket data...</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-orange-600">
                      {
                        associationRules.filter(
                          (rule) =>
                            rule.strength === "Strong" ||
                            rule.strength === "Very Strong"
                        ).length
                      }
                    </p>
                  </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Adjust thresholds to refine association rules
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    variant={
                      selectedMetric === "confidence" ? "default" : "outline"
                    }
                    size="sm"
                  ...
                  <Button
                    variant={
                      selectedMetric === "support" ? "default" : "outline"
                    }
                    size="sm"
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Product combinations with statistical significance (sorted by{" "}
              {selectedMetric})
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {associationRules
                    ...
                    .map((rule, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div className="text-sm font-medium">
                            {(rule.lift || 0).toFixed(2)}
                          </div>
                          <div className="text-xs text-gray-500">
                            {(rule.lift || 0) > 1 ? "Positive" : "Negative"}
                          </div>
                        </td>
                        <td className="py-3 px-4">
                          <Badge
                            variant="outline"
                            className={getStrengthColor(rule.strength)}
                          >
                            {rule.strength}
                          </Badge>
                        </td>
                        <td className="py-3 px-4">
                          <div className="text-sm font-medium text-green-600">
                            {formatCurrency(
                              rule.revenue_impact || rule.expectedRevenue || 0
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recommended product bundles for increased revenue
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-gray-600">
                          {opportunity.discount}% bundle discount
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>Revenue Opportunity:</strong> Cross-selling could
                    generate{" "}
                    {formatCurrency(
                      crossSellOpportunities.reduce(
                        (sum, opp) =>
                          sum +
                          (opp.potentialRevenue || opp.potential_revenue || 0),
                        0
                      )
                    )}{" "}
                    additional revenue
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    ...
                    {customerSegments[0]?.segment} segment shows highest value
                    with {formatCurrency(customerSegments[0]?.avgOrderValue)}{" "}
                    average order
                  </AlertDescription>
                </Alert>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <p className="text-sm text-green-800 mt-1">
                    Create "{crossSellOpportunities[0]?.bundle}" bundle with{" "}
                    {crossSellOpportunities[0]?.discount}% discount to drive
                    sales.
                  </p>
                </div>
                ...
                <div className="p-3 bg-purple-50 rounded-lg border border-purple-200">
                  ...
                  <p className="text-sm text-purple-800 mt-1">
                    Focus campaigns on {customerSegments[0]?.segment} segment
                    for maximum ROI.
                  </p>
                </div>
                ...
                <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <p className="text-sm text-yellow-800 mt-1">
                    Stock complementary products together based on association
                    rules.
                  </p>
                </div>
              </div>
        ...
        <div className="mt-6 bg-green-50 p-6 rounded-lg border border-green-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-green-800">
            <div>
              ...
              <p>
                How frequently items appear together relative to all
                transactions. Higher support indicates more common associations.
              </p>
            </div>
            <div>
              <h5 className="font-medium mb-2">Confidence</h5>
              <p>
                The likelihood that consequent items are purchased when
                antecedent items are bought. Measures rule reliability.
              </p>
            </div>
            <div>
              <h5 className="font-medium mb-2">Lift</h5>
              <p>
                How much more likely the consequent is to be bought when the
                antecedent is purchased. Lift {">"} 1 indicates positive
                association.
              </p>
            </div>
          </div>
